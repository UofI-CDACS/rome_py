...
class Station(Node):
    def __init__(self, name=None, loss_mode="lossy", depth=10):
        ...
        self.get_logger().info(
            f'Station "{self.this_station}" started with {loss_mode} mode, listening for parcels.'
        )
        # Publish kill signal AFTER subscriptions are set up
        kill_signal_pub = self.create_publisher(
            StationKill, f"{self.this_station}/kill", self.qos_profile
        )
        kill_signal = StationKill()
        kill_signal.kill_msg = (
            f"All other stations with this name ({self.this_station}) must die!"
        )
    ...
    def get_publisher(self, topic_name: str, qos_profile: QoSProfile):
        ...
        if qos_key not in self._pub_cache:
            self._pub_cache[qos_key] = self.create_publisher(
                Parcel, topic_name, qos_profile
            )
    ...
    async def log_parcel(self, parcel):
        ...
        database_data = {
            ...
            "sort_timestamp": (
                timestamp_recieved if timestamp_recieved is not None else timestamp_sent
            ),
        }
    ...
    def _on_parcel_received(self, parcel):
        ...
        self.get_logger().info(
            f"Parcel received callback triggered for parcel {parcel.parcel_id}"
        )
        try:
    ...
    def _on_kill_signal(self, kill_signal):
        ...
        self.get_logger().info(
            f"Received kill signal: {kill_signal.kill_msg} at {kill_signal.timestamp}"
        )
    ...
    async def parcel_callback(self, parcel: Parcel):
        ...
        self.get_logger().warn(
            f"Received parcel {parcel.parcel_id} but no handler implemented."
        )

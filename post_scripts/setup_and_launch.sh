...
launch_station_tmux_local() {
  ...
  remote_cmd+=" && (pgrep -f 'ros2 run post_core station' && pkill -f 'ros2 run post_core station' || true) >/dev/null 2>&1"
  remote_cmd+=" && echo 'About to run ros2 command...' >/dev/null 2>&1"
  remote_cmd+=" && sleep 2"
